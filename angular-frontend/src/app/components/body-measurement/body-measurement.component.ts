...
export class BodyMeasurementComponent implements OnInit, OnDestroy {
  ...
  loadSleepData(): void {
    ...
    this.apiService.getSleepData()
      ...
      .subscribe({
        ...
        error: () => {
          this.sleepLoading = false;
          this.snackBar.open('Fehler beim Laden der Schlafdaten', 'Schließen', { duration: 3000 });
        }
      });
  }
  ...
  saveSleep(): void {
    ...
    request$.pipe(takeUntil(this.destroy$)).subscribe({
      next: () => {
        ...
        this.editingSleepId = null;
        this.snackBar.open(isNew ? 'Schlafdaten gespeichert' : 'Schlafdaten aktualisiert', 'Schließen', { duration: 3000 });
        this.loadSleepData();
      },
      error: () => {
        this.sleepSaving = false;
  ...
  deleteSleepEntry(entry: SleepData): void {
    if (!entry.id) return;
    if (!confirm(`Schlafdaten vom ${this.formatDate(entry.recordedAt)} wirklich löschen?`)) return;
    this.apiService.deleteSleepData(entry.id)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: () => {
          this.snackBar.open('Schlafdaten gelöscht', 'Schließen', { duration: 3000 });
          this.loadSleepData();
        },
  ...
  runImport(): void {
    ...
    request$.pipe(takeUntil(this.destroy$)).subscribe({
      next: (result) => {
        ...
        this.importType = null;
        const msg = `${result.imported} Einträge importiert` +
      ...
      error: () => {
        this.importing = false;
        this.snackBar.open('Fehler beim Import', 'Schließen', { duration: 3000 });
      }
    });
  }
  ...
  private buildSleepTrendSeries(): void {
    // Map recordedAt → measuredAt so createTrendSeries can work with these entries
    ...
    this.sleepTrendSeries = [
      ...
      this.createTrendSeries(sorted, 'SpO₂', '%', '#42a5f5', (s) => s.spO2),
