...
### Fixed
- Heartbreak Hill leaderboard now shows one entry per participant (best time, identified by display name); re-uploading a faster run updates your time in place, slower runs are ignored.
...
- Admin import to define a challenge's segment geometry (start/end gates, polyline with elevation, distance/grade stats) from a reference GPX (`POST /api/admin/challenges/{slug}/define-from-gpx`).
- **Heartbreak Hill: shareable transparent 9:16 story image.** After an upload, the result panel renders a downloadable Instagram-story overlay (transparent PNG) on a client-side canvas — the climb's elevation curve, speed (ride km/h, run min/km), time and rank, plus a PACR logo tag. Three user-selectable templates (ridge silhouette, lower-third card, minimal), live preview, download and native share (mobile). Backend `SegmentEffortResultDto` now carries `avgSpeedKmh`/`avgPaceSecondsPerKm`; the frontend reuses a new `buildElevationPoints` helper for both the hero profile and the share canvas. New i18n keys `HEARTBREAK_HILL.SHARE_*` in EN and DE.
